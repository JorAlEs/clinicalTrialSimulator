#' Export an ADaM-style ADSL dataset
#'
...
#'
#' @param data A data frame containing patient-level data (after simulation of allocation, outcomes, etc.). This typically would be the data frame after calling the simulation functions.
#' @param study_id Character, an identifier for the study (used to construct \code{STUDYID} and \code{USUBJID}). Default is "STUDY1".
#'
#' @return A data frame with one row per patient, containing columns such as:
#' \itemize{
...
#'
#' @export
export_adam_format <- function(data, study_id = "STUDY1") {
  # Create STUDYID and USUBJID
  adsl <- data
  adsl$STUDYID <- study_id
  adsl$USUBJID <- paste0(study_id, "-", adsl$ID)

  # Determine desired column order
  preferred_order <- c("STUDYID", "USUBJID", "ID",
                       "Country", "Site", "Arm", "Age", "Sex",
                       "SeriousAE", "DropoutTime", "Dropped",
                       "EventTime", "EventStatus", "Outcome")
  col_order <- preferred_order[preferred_order %in% names(adsl)]
  adsl <- adsl[, col_order, drop = FALSE]

  # Ensure factors are not dropped in output (convert factors to character if needed)
  # (For simplicity, we will keep factors as they are; users can convert if necessary)
